/**
 * DBC Auth Utilities — OTP authentication helpers
 * Requires: frontend/config.js and frontend/pg-utils.js loaded first
 *
...
async function requestOTPCode(email, purpose) {
    ...
    const res = await fetch(`${PureGro_CONFIG.API_URL}/auth/otp/request`, {
        method: 'POST',
...
async function resendOTPCode(email, purpose) {
    if (!purpose) purpose = 'login';
    const res = await fetch(`${PureGro_CONFIG.API_URL}/auth/otp/resend`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: email, purpose: purpose })
    });
...
async function verifyOTPCode(email, otpCode) {
    const res = await fetch(`${PureGro_CONFIG.API_URL}/auth/otp/verify`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
async function fetchCurrentUser(token) {
    ...
    try {
        let res = await fetch(`${PureGro_CONFIG.API_URL}/auth/otp/me`, {
            headers: { 'Authorization': `Bearer ${token}` }
        });

        if (!res.ok) {
            res = await fetch(`${PureGro_CONFIG.API_URL}/auth/otp/me`, {
                headers: { 'Authorization': `Bearer ${token}` }
            });
        }
...
async function updateUserProfile(profileData, token) {
    if (!token) token = getToken();
    const res = await fetch(`${PureGro_CONFIG.API_URL}/auth/otp/update-profile`, {
        method: 'PUT',
        headers: {
