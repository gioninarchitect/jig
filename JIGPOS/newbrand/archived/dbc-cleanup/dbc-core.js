 ...
 * Requires (in order):
 *   1. frontend/config.js      (PureGro_CONFIG, API_URL, DEV_MODE)
 *   2. frontend/pg-utils.js   (showToast, getToken, apiCall, logout, formatCurrency)
 *   3. frontend/pg-auth.js    (OTP helpers, token setters, isAuthenticated)
 *
 * Provides: window.PG — single API surface for all pages and future React bridge.
 */
 ...
 */
function _pgShowConfirm(message, onConfirm, options) {
    if (!options) options = {};
    ...
    var existing = document.getElementById('puregro-confirm-overlay');
    if (existing) existing.remove();

    var overlay = document.createElement('div');
    overlay.id = 'puregro-confirm-overlay';
    overlay.style.cssText = 'position:fixed;top:0;left:0;width:100%;height:100%;' +
        'background:rgba(0,0,0,0.6);display:flex;align-items:center;justify-content:center;' +
        'z-index:300000;font-family:"Barlow",sans-serif;animation:dbcConfirmIn 0.2s ease-out;';
    ...
    overlay.innerHTML =
        ...
        'width:90%;box-shadow:0 16px 48px rgba(0,0,0,0.3);text-align:center;">' +
            ...
            '<div style="display:flex;gap:12px;justify-content:center;">' +
                '<button id="puregro-confirm-cancel" style="padding:10px 24px;border:1px solid #ccc;' +
                ...
                '<button id="puregro-confirm-ok" style="padding:10px 24px;border:none;' +
    ...
    if (!document.getElementById('puregro-confirm-keyframes')) {
        var style = document.createElement('style');
        style.id = 'puregro-confirm-keyframes';
        style.textContent =
    ...
    // Wire buttons
    document.getElementById('puregro-confirm-cancel').addEventListener('click', function() {
        overlay.remove();
    });
    document.getElementById('puregro-confirm-ok').addEventListener('click', function() {
        overlay.remove();
 ...
 */
function _dbcShowPrompt(message, onSubmit, options) {
    if (!options) options = {};
    ...
    var existing = document.getElementById('puregro-prompt-overlay');
    if (existing) existing.remove();

    var overlay = document.createElement('div');
    overlay.id = 'puregro-prompt-overlay';
    overlay.style.cssText = 'position:fixed;top:0;left:0;width:100%;height:100%;' +
        'background:rgba(0,0,0,0.6);display:flex;align-items:center;justify-content:center;' +
        'z-index:300000;font-family:"Barlow",sans-serif;animation:dbcConfirmIn 0.2s ease-out;';
    ...
    overlay.innerHTML =
        ...
        'width:90%;box-shadow:0 16px 48px rgba(0,0,0,0.3);text-align:center;">' +
            ...
            message + '</p>' +
            '<textarea id="puregro-prompt-input" rows="3" placeholder="' + placeholder + '" ' +
            ...
            '<div style="display:flex;gap:12px;justify-content:center;">' +
                '<button id="puregro-prompt-cancel" style="padding:10px 24px;border:1px solid #ccc;' +
                'background:transparent;border-radius:8px;cursor:pointer;font-size:0.9rem;' +
                'color:' + B.darkGreen + ';font-family:\'Inter\',sans-serif;">' + cancelText + '</button>' +
                '<button id="puregro-prompt-ok" style="padding:10px 24px;border:none;' +
    ...
    var inputEl = document.getElementById('puregro-prompt-input');
    inputEl.focus();

    document.getElementById('puregro-prompt-cancel').addEventListener('click', function() {
        overlay.remove();
    });
    document.getElementById('puregro-prompt-ok').addEventListener('click', function() {
...
function _dbcShowLoading(message) {
    ...
    var B = PureGro_CONFIG.BRAND;
    var overlay = document.createElement('div');
    overlay.id = 'puregro-loading-overlay';
    overlay.style.cssText = 'position:fixed;top:0;left:0;width:100%;height:100%;' +
...
function _dbcHideLoading() {
    var el = document.getElementById('puregro-loading-overlay');
    if (el) el.remove();
}
...
// ============================================
// PureGro Namespace — single API surface
// ============================================

var DBC = {
    config: PureGro_CONFIG,
    ...
    ui: {
        ...
        showConfirm: _pgShowConfirm,
        showPrompt: _dbcShowPrompt,
