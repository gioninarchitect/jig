...
function showToast(arg1, arg2, arg3) {
    ...
    document.querySelectorAll('.puregro-shared-toast').forEach(t => t.remove());
    ...
    const colors = {
        success: { bg: PureGro_CONFIG.BRAND.green, border: PureGro_CONFIG.BRAND.gold },
        error: { bg: PureGro_CONFIG.BRAND.darkRed, border: PureGro_CONFIG.BRAND.red },
        warning: { bg: '#7A6520', border: PureGro_CONFIG.BRAND.gold },
        info: { bg: PureGro_CONFIG.BRAND.darkGreen, border: PureGro_CONFIG.BRAND.green }
    };

    const c = colors[type] || colors.info;

    const toast = document.createElement('div');
    toast.className = 'puregro-shared-toast';
    ...
    toast.innerHTML = `
        <div style="display:flex;align-items:center;gap:12px;">
            ...
            <button onclick="this.closest('.puregro-shared-toast').remove()" style="background:none;border:none;color:inherit;font-size:1.25rem;cursor:pointer;padding:4px;line-height:1;flex-shrink:0;">&times;</button>
        </div>
    `;

    toast.style.cssText = `
        position:fixed;top:20px;right:20px;
        background:${c.bg};color:${PureGro_CONFIG.BRAND.cream};
    ...
    if (!document.getElementById('puregro-toast-keyframes')) {
        const style = document.createElement('style');
        style.id = 'puregro-toast-keyframes';
        style.textContent = `
...
function formatCurrency(amount) {
    return `${PureGro_CONFIG.CURRENCY.symbol}${(Number(amount) || 0).toFixed(2)}`;
}
...
async function apiCall(endpoint, method, data) {
    ...
    const url = endpoint.startsWith('http') ? endpoint : `${PureGro_CONFIG.API_URL}${endpoint}`;
