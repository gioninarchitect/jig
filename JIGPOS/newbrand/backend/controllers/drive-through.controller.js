// Drive-Through (Retail Store) Controller — Business logic extracted from routes/drive-through.js
    ...
    try {
        ...
        if (!allowedPaymentMethods.includes(payment.method)) {
            return res.status(400).json({
                success: false,
                error: 'Invalid payment method. Only InstaPay, EFT, or Card payments are accepted for retail-store orders.'
            });
        }
...
exports.getQueue = async (req, res) => {
    try {
        ...
        logger.error('Get queue error', { error: error.message, stack: error.stack });
        res.status(500).json({
            success: false,
            error: error.message
        });
...
exports.getOrder = async (req, res) => {
    try {
        ...
        const isStaff = req.user.role === 'admin' || req.user.role === 'branch_manager' || req.user.role === 'branch_assistant';
...
exports.updateLocation = async (req, res) => {
    try {
        ...
        const storeLocation = { lat: -26.0287, lng: 28.0022 };
...
exports.updateStatus = async (req, res) => {
    try {
        ...
        logger.error('Status update error', { error: error.message, stack: error.stack });
        res.status(500).json({
            success: false,
            error: error.message
        });
...
exports.notifyArrival = async (req, res) => {
    try {
        ...
        res.json({
            ...
            laneAssignment: 'Lane 1',
            requiresSection21Verification: order.compliance.requiresSection21
        });

    } catch (error) {
        logger.error('Arrival notification error', { error: error.message, stack: error.stack });
        res.status(500).json({
            success: false,
            error: error.message
        });
...
exports.completePickup = async (req, res) => {
    try {
        ...
        logger.error('Complete order error', { error: error.message, stack: error.stack });
        res.status(500).json({
            success: false,
            error: error.message
        });
...
// Get customer's retail-store order history
...
exports.getPendingApprovals = async (req, res) => {
    try {
        ...
        const isAuthorized = req.user.role === 'admin' || req.user.role === 'branch_manager';
        if (!isAuthorized) {
            return res.status(403).json({
                success: false,
        ...
        logger.error('Get pending approvals error', { error: error.message, stack: error.stack });
        res.status(500).json({
            success: false,
            error: error.message
        });
...
// Approve EFT payment
exports.approveEFT = async (req, res) => {
    try {
        const { orderId } = req.params;
        ...
        logger.error('Approve EFT error', { error: error.message, stack: error.stack });
        res.status(500).json({
            success: false,
            error: error.message
        });
...
// Reject EFT payment
exports.rejectEFT = async (req, res) => {
    try {
        const { orderId } = req.params;
        ...
        // Trigger WebSocket notification to customer
        websocket.notifyPaymentRejected({
            customerId: order.customerId,
            orderId: order._id,
            orderNumber: order.orderNumber,
        ...
        logger.error('Reject EFT error', { error: error.message, stack: error.stack });
        res.status(500).json({
            success: false,
            error: error.message
        });
