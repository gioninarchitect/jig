/**
 * Staff Controller — Business logic for staff management
 */
...
const STAFF_PERMISSIONS = {
  branch_manager: [
    ...
    'manage_pos_sync',         // Sync cafe menus
...
exports.getAll = async (req, res) => {
  try {
    ...
    const query = {
      role: { $in: ['branch_manager', 'branch_assistant'] }
    };
...
// POST / — Create staff member with permissions from STAFF_PERMISSIONS
exports.create = async (req, res) => {
  try {
    ...
    if (!role.startsWith('staff_')) {
      return res.status(400).json({
        success: false,
        message: 'Invalid staff role. Must be branch_manager or branch_assistant'
      });
    }
...
// DELETE /:id — Deactivate or permanently delete staff member
...
// GET /meta/roles — List roles with permissions
    ...
    const roles = [
      {
        ...
        permissions: STAFF_PERMISSIONS.branch_manager
      },
      {
        value: 'branch_assistant',
        ...
        permissions: STAFF_PERMISSIONS.branch_assistant
      }
    ];
