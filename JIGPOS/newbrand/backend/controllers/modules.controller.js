// Modules Controller — Business logic for module marketplace
const logger = require('../modules/logger');
const { Module, ModuleInstallation } = require('../modules/database/models/Module');

// GET / — List available/beta modules (admin only)
...
// GET /installed — Get installed modules for current business
async function getInstalled(req, res) {
    try {
        ...
        logger.error('Get installed modules error', { error: error.message, stack: error.stack });
        res.status(500).json({
            success: false,
...
// POST /install/:moduleId — Install/purchase a module with trial (admin only)
async function install(req, res) {
    try {
        ...
        logger.error('Install module error', { error: error.message, stack: error.stack });
        res.status(500).json({
            success: false,
...
// DELETE /uninstall/:moduleId — Uninstall a module (admin only)
async function uninstall(req, res) {
    try {
        ...
        logger.error('Uninstall module error', { error: error.message, stack: error.stack });
        res.status(500).json({
            success: false,
...
// PUT /configure/:moduleId — Update module configuration (admin only)
async function configure(req, res) {
    try {
        ...
        logger.error('Configure module error', { error: error.message, stack: error.stack });
        res.status(500).json({
            success: false,
...
// POST /activate/:moduleId — Activate subscription after payment (admin only)
async function activate(req, res) {
    try {
        ...
        logger.error('Activate module error', { error: error.message, stack: error.stack });
        res.status(500).json({
            success: false,
...
// POST /track-usage/:moduleId — Track module usage
async function trackUsage(req, res) {
    try {
        ...
        logger.error('Track usage error', { error: error.message, stack: error.stack });
        res.status(500).json({
            success: false,
