/**
 * Voucher Controller — Business logic for lifestyle/retail voucher system
 *
...
// Validate voucher code before applying to cart (public)
exports.validate = async (req, res) => {
  try {
    ...
    if (!cartTotal || !cartItems || !Array.isArray(cartItems)) {
      return res.status(400).json({ success: false, error: 'Cart total and items are required' });
    }
    ...
    res.status(500).json({ success: false, error: 'Error validating voucher' });
  }
};

// Redeem voucher after order completion
exports.redeem = async (req, res) => {
  try {
    ...
    if (!orderId || discountApplied === undefined || !orderTotal) {
      return res.status(400).json({ success: false, error: 'Order ID, discount amount, and order total are required' });
    }
    ...
    res.json({ success: true, message: 'Voucher redeemed successfully', redeemCount: voucher.redeemCount });
  } catch (error) {
    logger.error('Redeem voucher error:', error);
    res.status(500).json({ success: false, error: 'Error redeeming voucher' });
  }
};
...
exports.create = async (req, res) => {
  try {
    ...
    const {
      ...
      startDate, expiryDate, redemptionLimit, maxRedemptions,
      autoApplyTiers, applicableCategories, excludedCategories, internalNotes
    } = req.body;

    if (!name || !type || value === undefined || !expiryDate) {
      return res.status(400).json({ success: false, error: 'Name, type, value, and expiry date are required' });
    }

    const voucher = new Voucher({
      code: code || Voucher.generateCode(),
      name, description, type, value, minPurchase, maxDiscount,
      startDate, expiryDate: new Date(expiryDate),
      redemptionLimit, maxRedemptions, autoApplyTiers,
    ...
    res.json({
      ...
      voucher: {
        id: voucher._id, code: voucher.code, name: voucher.name,
        type: voucher.type, value: voucher.value, expiryDate: voucher.expiryDate
      }
    });
  } catch (error) {
    ...
    res.status(500).json({ success: false, error: 'Error creating voucher', details: error.message });
  }
};
...
exports.update = async (req, res) => {
  try {
    ...
    if (voucher.redeemCount > 0 && req.body.code) {
      return res.status(400).json({ success: false, error: 'Cannot change code of redeemed voucher' });
    }
    ...
    res.status(500).json({ success: false, error: 'Error updating voucher' });
  }
};

// Deactivate voucher (admin only, soft delete)
exports.deactivate = async (req, res) => {
  try {
    ...
    if (voucher.redeemCount > 0) {
      return res.status(400).json({ success: false, error: 'Cannot delete voucher that has been redeemed. Use deactivate instead.' });
    }
    ...
    res.status(500).json({ success: false, error: 'Error deactivating voucher' });
  }
};

// Get voucher analytics (admin only)
exports.getAnalytics = async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      analytics: {
        code: voucher.code, name: voucher.name, status: voucher.status,
        totalRedemptions: voucher.redeemCount, uniqueCustomers,
        totalDiscountGiven: totalDiscount, totalRevenue,
