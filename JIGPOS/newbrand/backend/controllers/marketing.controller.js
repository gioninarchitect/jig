// Marketing Controller — Business logic for campaigns and marketing preferences
...
exports.getCampaigns = async (req, res) => {
  try {
    const { page = 1, limit = 20, status, type, tag, sort = '-createdAt' } = req.query;
...
exports.getCampaignStats = async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: 'Error fetching campaign statistics' });
  }
};
...
exports.getCampaignDetailedStats = async (req, res) => {
  try {
    const campaign = await Campaign.findById(req.params.id);
    if (!campaign) {
      return res.status(404).json({ success: false, message: 'Campaign not found' });
...
exports.updateCampaign = async (req, res) => {
  try {
    ...
    if (!['draft', 'scheduled'].includes(campaign.status)) {
      return res.status(400).json({ success: false, message: 'Only draft or scheduled campaigns can be edited' });
    }
...
exports.scheduleCampaign = async (req, res) => {
  try {
    ...
    res.json({ success: true, message: 'Campaign scheduled', campaign });
  } catch (error) {
    logger.error('Schedule campaign error', { error: error.message, stack: error.stack });
    res.status(500).json({ success: false, message: error.message || 'Error scheduling campaign' });
  }
};
...
exports.sendCampaign = async (req, res) => {
  try {
    ...
    if (!['draft', 'scheduled'].includes(campaign.status)) {
      return res.status(400).json({ success: false, message: 'Campaign cannot be sent in current status' });
    }
    ...
    logger.info('Campaign sending started', {
      campaignId: campaign.campaignId, recipientCount: recipients.length, sentBy: req.user.id
    });
    ...
    res.json({ success: true, message: `Campaign sent to ${recipients.length} recipients`, campaign });
  } catch (error) {
...
exports.cancelCampaign = async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: error.message || 'Error cancelling campaign' });
  }
};
...
exports.getPreferenceStats = async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: 'Error fetching subscription statistics' });
  }
};
...
exports.getPreferences = async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: 'Error fetching marketing preferences' });
  }
};
...
exports.updatePreferences = async (req, res) => {
  try {
    ...
    allowedUpdates.forEach(field => {
      if (req.body[field] !== undefined) preference[field] = req.body[field];
    });
    ...
    res.json({ success: true, message: 'Preferences updated', preference });
  } catch (error) {
...
exports.unsubscribe = async (req, res) => {
  try {
    ...
    if (!email && !token) {
      return res.status(400).json({ success: false, message: 'Email or unsubscribe token is required' });
    }
...
exports.resubscribe = async (req, res) => {
  try {
    ...
    res.json({ success: true, message: 'Successfully resubscribed', preference });
  } catch (error) {
    logger.error('Resubscribe error', { error: error.message, stack: error.stack });
    res.status(500).json({ success: false, message: 'Error processing resubscribe request' });
  }
};
