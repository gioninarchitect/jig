 ...
 * Handles module subscription lifecycle: catalog, subscribe, cancel, webhooks, admin
 */
...
// GET /modules - List available modules (public)
...
// GET /my - Get user's active subscriptions (auth)
...
// POST /subscribe - Subscribe to a module (auth)
...
// POST /:id/cancel - Cancel subscription (auth)
...
// GET /:moduleId/check - Check if user has access to module (auth)
...
// POST /webhook/payu - Handle PayU webhook notifications (public)
const webhookPayu = async (req, res) => {
  try {
...
// GET /admin/all - List all subscriptions (auth+admin)
...
// POST /admin/:id/suspend - Suspend subscription (auth+admin)
const adminSuspend = async (req, res) => {
  try {
    const { id } = req.params;
    const { reason } = req.body;
