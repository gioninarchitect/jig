// Batch Controller — Business logic for lot tracking with cannabinoid profiles
...
exports.getAll = async (req, res) => {
  try {
    const { page = 1, limit = 20, product, supplier, status, qaStatus, branch, sort = '-createdAt' } = req.query;
  ...
  try {
    ...
    res.json({
      ...
      trace: {
        batch,
        summary: {
          batchId: batch.batchId, product: batch.product?.name,
          supplier: batch.supplier?.name, supplierLicense: batch.supplier?.license?.number,
          initialQuantity: batch.initialQuantity, remainingQuantity: batch.remainingQuantity,
          soldQuantity: batch.initialQuantity - batch.remainingQuantity,
          totalMovements: batch.movements?.length || 0, totalSales: batch.sales?.length || 0,
          qaStatus: batch.qaStatus, expiryDate: batch.expiryDate
        }
      }
    });
  } catch (error) {
    logger.error('Get batch trace error', { error: error.message, stack: error.stack });
    res.status(500).json({ success: false, message: 'Error fetching batch trace' });
  }
};
...
exports.create = async (req, res) => {
  try {
    const {
      product, supplier, cannabinoids, terpenes, harvestDate, testDate, expiryDate,
      labCertificateUrl, labName, initialQuantity, unitOfMeasure, currentLocation,
      purchaseOrder, unitCost, notes
    } = req.body;
    ...
    const batch = new Batch({
      product, supplier, cannabinoids, terpenes,
      harvestDate: harvestDate ? new Date(harvestDate) : undefined,
      testDate: testDate ? new Date(testDate) : undefined,
      expiryDate: expiryDate ? new Date(expiryDate) : undefined,
      labCertificateUrl, labName, initialQuantity, remainingQuantity: initialQuantity,
      unitOfMeasure: unitOfMeasure || 'grams', currentLocation, purchaseOrder,
      unitCost, totalCost: unitCost ? unitCost * initialQuantity : undefined,
...
exports.update = async (req, res) => {
  try {
    ...
    const allowedUpdates = [
      'cannabinoids', 'terpenes', 'harvestDate', 'testDate', 'expiryDate',
      'labCertificateUrl', 'labName', 'currentLocation', 'unitCost', 'notes'
    ];
...
exports.qaApprove = async (req, res) => {
  try {
    ...
    if (batch.qaStatus !== 'pending') {
      return res.status(400).json({ success: false, message: `Batch cannot be approved. Current QA status: ${batch.qaStatus}` });
    }
    ...
    res.json({ success: true, message: 'Batch approved successfully', batch });
  } catch (error) {
    logger.error('QA approve batch error', { error: error.message, stack: error.stack });
    res.status(500).json({ success: false, message: 'Error approving batch' });
  }
};

// Reject batch (QA rejection)
exports.qaReject = async (req, res) => {
  try {
    ...
    if (batch.qaStatus !== 'pending') {
      return res.status(400).json({ success: false, message: `Batch cannot be rejected. Current QA status: ${batch.qaStatus}` });
    }
    ...
    res.json({ success: true, message: 'Batch rejected', batch });
  } catch (error) {
...
exports.move = async (req, res) => {
  try {
    ...
    if (!toBranch || !quantity) {
      return res.status(400).json({ success: false, message: 'toBranch and quantity are required' });
    }
    ...
    await batch.recordMovement(
      fromBranch || batch.currentLocation, toBranch, quantity, req.user.id, transferId, reason
    );

    logger.info('Batch movement recorded', {
      batchId: batch.batchId, from: fromBranch, to: toBranch, quantity, movedBy: req.user.id
    });

    res.json({ success: true, message: 'Batch movement recorded', batch });
  } catch (error) {
    logger.error('Record batch movement error', { error: error.message, stack: error.stack });
    res.status(500).json({ success: false, message: error.message || 'Error recording batch movement' });
  }
};
...
exports.getExpiring = async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: 'Error fetching expiring batches' });
  }
};
...
exports.getLowStock = async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: 'Error fetching low stock batches' });
  }
};
