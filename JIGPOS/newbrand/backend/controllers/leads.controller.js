...
// GET / — List leads with pagination, filtering, search, status counts (auth+role)
...
// GET /:id — Single lead with populate (auth+role)
const getById = async (req, res) => {
  try {
    ...
    logger.error('Get lead error', { error: error.message, stack: error.stack });
    res.status(500).json({ success: false, message: 'Error fetching lead' });
  }
};

// PUT /:id — Update lead status/assignedTo/notes, validates assignee is staff (auth+role)
  ...
  try {
    ...
    if (assignedTo !== undefined) {
      if (assignedTo) {
        ...
        // Only allow assignment to staff members (admin, branch_manager, branch_assistant)
        if (!['admin', 'branch_manager', 'branch_assistant'].includes(assignee.role)) {
...
// DELETE /:id — Delete lead (auth+admin)
const remove = async (req, res) => {
  try {
    ...
    logger.error('Delete lead error', { error: error.message, stack: error.stack });
    res.status(500).json({ success: false, message: 'Error deleting lead' });
  }
};

// GET /meta/statistics — Lead statistics with aggregation (auth+role)
