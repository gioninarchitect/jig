...
const retailStoreOrderSchema = new mongoose.Schema({
    orderId: {
        type: String,
        required: true,
        unique: true,
...
retailStoreOrderSchema.index({ status: 1, 'queue.addedAt': 1 });
retailStoreOrderSchema.index({ customerId: 1, status: 1 });
retailStoreOrderSchema.index({ createdAt: -1 });

// Virtual for checking if order has any Section 21 products
retailStoreOrderSchema.virtual('hasSection21Products').get(function() {
    return this.products.some(p => p.requiresSection21);
});

// Method to calculate ETA based on queue position
retailStoreOrderSchema.methods.calculateETA = function() {
...
retailStoreOrderSchema.methods.addAuditEntry = function(action, performedBy, details) {
...
retailStoreOrderSchema.statics.getActiveQueue = function() {
    return this.find({
...
retailStoreOrderSchema.statics.getNextQueuePosition = async function() {
...
const DriveThrough = mongoose.model('DriveThrough', retailStoreOrderSchema);
