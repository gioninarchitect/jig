...
stockTakeSchema.methods.calculateSummary = function() {
  ...
  // Quick Count Mode: photos required for weighable items + high-variance countable items
  this.allPhotosUploaded = this.lineItems.every(item => {
    // Weighable items always need scale photo
    if (!item.isCountable) return !!item.scalePhoto?.url;
    // Countable items: photo only required if variance > 20%
    if (item.actualQty !== undefined && item.actualQty !== null && item.expectedQty > 0) {
      const varPct = Math.abs(((item.actualQty - item.expectedQty) / item.expectedQty) * 100);
      if (varPct > 20) return item.unitPhotos?.length > 0;
    }
    return true; // No photo needed for countable items with acceptable variance
  });
...
stockTakeSchema.methods.validateLineItem = function(index) {
  ...
  // Weighable items always require scale photo (OCR reads the weight)
  if (!item.scalePhoto?.url && !item.isCountable) {
    errors.push('Scale photo is required');
  }

  // Quick Count Mode: countable items only require photo if variance > 20%
  // Photo is optional for low-variance countable items
  if (item.isCountable && (!item.unitPhotos || item.unitPhotos.length === 0)) {
    // Calculate variance to determine if photo is needed
    if (item.actualQty !== undefined && item.actualQty !== null && item.expectedQty > 0) {
      const varPct = Math.abs(((item.actualQty - item.expectedQty) / item.expectedQty) * 100);
      if (varPct > 20) {
        errors.push('Photo required for countable items with >20% variance');
      }
    }
    // If no count entered yet, don't flag photo as error
  }
