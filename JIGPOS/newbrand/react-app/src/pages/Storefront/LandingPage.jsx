...
const LOCATIONS_PREVIEW = [
  { name: 'Potchefstroom', address: 'Potchefstroom', province: 'North West', hours: 'Mon - Sat: 9:00 - 17:00', open: true },
  { name: 'Online Store', address: 'origin.cleva-ai.co.za', province: 'Nationwide Delivery', hours: '24/7', open: true },
];
  ...
  return (
    <div>
      ...
      <section
        ...
        style={{ background: 'linear-gradient(160deg, #C9A84C 0%, #8B6914 40%, #1A1A1A 100%)' }}
      >
        ...
        <div className="max-w-[680px] mx-auto px-4 py-24 sm:py-36 text-center relative z-10">
          ...
          <h1 className="font-heading text-[4rem] sm:text-[5rem] text-white uppercase leading-[0.95] mb-2" style={{ textShadow: '0 4px 30px rgba(212,175,55,0.4)' }}>
            Origin <span className="font-[900]">Cannabis</span>
          </h1>

          {/* Tagline */}
          <p className="font-accent italic text-[1.1rem] sm:text-[1.35rem] mb-6" style={{ color: '#F0A500' }}>
            "Quality Counts &middot; Finest Goods"
          </p>
          ...
          <div className="flex flex-wrap justify-center gap-4 mb-12">
            <Link
              ...
              style={{ background: 'linear-gradient(135deg, #E8C45A 0%, #F0A500 50%, #B8922D 100%)', color: '#1A1A1A', boxShadow: '0 15px 40px rgba(212,175,55,0.3)' }}
            >
          ...
          <div className="flex justify-center gap-12 sm:gap-16">
            {STATS.map((stat) => (
              <div key={stat.label} className="text-center">
                <div className="font-heading text-[3rem] sm:text-[3.5rem] leading-none" style={{ color: '#F0A500', textShadow: '0 4px 20px rgba(212,175,55,0.4)' }}>{stat.value}</div>
      ...
      {/* ══════════ HOW IT WORKS ══════════ */}
      <section id="how-it-works" className="py-16 sm:py-24 bg-origin-slate">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <div className="text-[0.75rem] font-bold uppercase tracking-[0.2em] mb-2" style={{ color: '#B8922D' }}>Quality Counts</div>
            <h2 className="font-heading text-[2.5rem] sm:text-[3.5rem] text-or-gold uppercase">Getting Started</h2>
          ...
          <div className="relative sm:contents">
            ...
            <div className="absolute left-0 top-0 bottom-0 w-8 z-10 pointer-events-none sm:hidden" style={{ background: 'linear-gradient(to right, #0E0E0E, transparent)' }} />
            <div className="absolute right-0 top-0 bottom-0 w-8 z-10 pointer-events-none sm:hidden" style={{ background: 'linear-gradient(to left, #0E0E0E, transparent)' }} />
            ...
            >
              {STEPS.map((step) => (
                ...
                >
                  ...
                  <div className="absolute -top-4 left-1/2 -translate-x-1/2 w-9 h-9 rounded-full flex items-center justify-center font-heading text-lg z-10" style={{ background: 'linear-gradient(135deg, #E8C45A, #F0A500)', color: '#8B6914', boxShadow: '0 8px 20px rgba(212,175,55,0.4)' }}>
                    {step.num}
                  </div>
                  <div className="w-[80px] h-[80px] rounded-[20px] flex items-center justify-center mx-auto mt-2 mb-6 text-[2rem] transition-all group-hover:scale-105" style={{ background: 'linear-gradient(145deg, #C9A84C, #8B6914)', boxShadow: '0 15px 35px rgba(45,90,61,0.3), inset 0 1px 0 rgba(255,255,255,0.1)', transform: 'translateZ(20px)' }}>
                    {step.emoji}
                  </div>
                  <h3 className="font-heading text-xl sm:text-[1.4rem] uppercase text-or-gold tracking-wide mb-2">{step.title}</h3>
              ...
              {STEPS.map((_, i) => (
                <div
                  ...
                  style={{
                    ...
                    height: '8px',
                    background: activeStep === i ? '#F0A500' : 'rgba(58,95,72,0.2)',
                  }}
                />
              ))}
            </div>
      ...
      {/* ══════════ FEATURED PRODUCTS ══════════ */}
      <section id="menu" className="py-16 sm:py-24" style={{ background: '#1A1A1A' }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-end justify-between mb-10">
            <div>
              <div className="text-[0.75rem] font-bold uppercase tracking-[0.2em] mb-2" style={{ color: '#B8922D' }}>Origin by ILCO Farming Collection</div>
            ...
            <Link to="/products" className="hidden sm:inline-flex items-center gap-2 px-6 py-3 text-sm font-bold uppercase tracking-wider rounded-xl hover:-translate-y-1 transition-all" style={{ background: 'linear-gradient(135deg, #E8C45A, #F0A500, #B8922D)', color: '#1A1A1A' }}>
              View All
            </Link>
          </div>
          ...
          {loading ? (
            <div className="flex justify-center py-12">
              <div className="w-10 h-10 rounded-full animate-spin" style={{ border: '4px solid rgba(255,255,255,0.2)', borderTopColor: '#F0A500' }} />
            </div>
          ) : featured.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {featured.map((product) => {
                ...
                return (
                  ...
                  >
                    {/* Image area */}
                    <div className="relative h-[160px] overflow-hidden" style={{ background: 'linear-gradient(145deg, #4A7A5D, #C9A84C)' }}>
                      {product.category && (
                        <span className="absolute top-3 right-3 z-10 px-3 py-1 rounded-full text-[0.7rem] font-bold uppercase" style={{ background: 'linear-gradient(135deg, #E8C45A, #F0A500)', color: '#1A1A1A' }}>
                          {product.category}
                        </span>
                      )}
                      {product.images?.[0] || product.image ? (
                        ...
                        <div className="w-full h-full flex items-center justify-center">
                          <img src="/app/images/origin-logo.png" alt="Origin" className="w-20 h-20 object-contain opacity-60" />
                        </div>
                      )}
                    ...
                    {/* Content */}
                    <div className="p-5">
                      <h3 className="font-heading text-[1.35rem] uppercase text-or-gold truncate">{product.name}</h3>
                      ...
                      <button
                        ...
                        style={{ background: 'linear-gradient(135deg, #E8C45A, #F0A500, #B8922D)', color: '#1A1A1A', boxShadow: '0 4px 12px rgba(212,175,55,0.3)' }}
                      >
          ...
          <div className="text-center mt-8 sm:hidden">
            <Link to="/products" className="text-sm font-semibold text-or-gold hover:text-or-gold-dark">View All Products &rarr;</Link>
          </div>
        </div>
      </section>
      ...
      <section className="py-16 sm:py-20 bg-origin-slate">
        <div className="max-w-3xl mx-auto px-4 text-center">
          <div className="rounded-[20px] p-10 sm:p-14" style={{ background: 'linear-gradient(160deg, #C9A84C 0%, #8B6914 40%, #1A1A1A 100%)' }}>
            ...
            <Link
              to="/register"
              className="inline-flex items-center gap-2 px-8 py-4 text-sm font-bold uppercase tracking-wider rounded-xl hover:-translate-y-1 transition-all"
              style={{ background: 'linear-gradient(135deg, #E8C45A, #F0A500, #B8922D)', color: '#1A1A1A', boxShadow: '0 15px 40px rgba(212,175,55,0.3)', border: '2px solid #F0A500' }}
            >
              Become a Member
            </Link>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {FEATURES.map((feat) => (
              ...
              >
                <div className="w-[70px] h-[70px] rounded-[20px] flex items-center justify-center mx-auto mb-5 text-[1.8rem] group-hover:scale-110 transition-transform" style={{ background: 'linear-gradient(145deg, #E8C45A, #F0A500)', boxShadow: '0 10px 25px rgba(212,175,55,0.3)' }}>
                  {feat.emoji}
                </div>
      ...
      {/* ══════════ LOCATIONS ══════════ */}
      <section id="locations" className="py-16 sm:py-24 bg-origin-slate" style={{ perspective: '1000px' }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-10">
            ...
            <p className="font-accent italic text-gray-500 mt-2">Visit us in Potchefstroom or shop online</p>
          </div>
          ...
          <div className="relative overflow-x-auto pb-6 -mx-4 px-4 sm:mx-0 sm:px-0 scrollbar-hide">
            ...
            <div className="absolute left-0 top-0 bottom-0 w-8 z-10 pointer-events-none sm:hidden" style={{ background: 'linear-gradient(to right, #0E0E0E, transparent)' }} />
            <div className="absolute right-0 top-0 bottom-0 w-8 z-10 pointer-events-none sm:hidden" style={{ background: 'linear-gradient(to left, #0E0E0E, transparent)' }} />
            <div className="flex gap-6 sm:grid sm:grid-cols-2 sm:gap-6 min-w-max sm:min-w-0 pt-4">
              {LOCATIONS_PREVIEW.map((loc) => (
                <Link
                  ...
                  className={`group relative bg-white rounded-[24px] p-8 pt-10 text-center border transition-all duration-500 w-[260px] sm:w-auto shrink-0 sm:shrink hover:-translate-y-3 ${
                    loc.open
                      ? 'border-2 border-or-gold cursor-pointer'
                ...
                >
                  {/* Top gradient bar */}
                  <div className="absolute top-0 left-[10%] right-[10%] h-[5px] rounded-b-full bg-gradient-to-r from-or-gold via-or-gold to-origin-red scale-x-0 group-hover:scale-x-100 transition-transform duration-500 origin-left" />

                  {/* Status badge */}
                  <div className={`absolute -top-3 left-1/2 -translate-x-1/2 px-4 py-1 rounded-full text-[0.7rem] font-bold uppercase tracking-wider whitespace-nowrap ${loc.open ? 'text-white' : 'text-white'}`} style={{ background: loc.open ? '#F0A500' : '#737373' }}>
                    {loc.open ? 'Now Open' : 'Coming Soon'}
                  </div>

                  {/* Location icon */}
                  <div className="w-[70px] h-[70px] rounded-full flex items-center justify-center mx-auto mb-5 text-[1.5rem] group-hover:scale-110 transition-transform" style={{ background: 'linear-gradient(135deg, #C9A84C, #8B6914)', boxShadow: '0 8px 20px rgba(58,95,72,0.3)', transform: 'translateZ(15px)' }}>
                  ...
                  <h3 className={`font-heading text-2xl uppercase mb-2 ${loc.open ? 'text-or-gold' : 'text-or-gold'}`}>{loc.name}</h3>
                  <p className="text-sm text-gray-500 mb-4 leading-relaxed">{loc.address}<br />{loc.province}</p>

                  {loc.hours && (
                    <div className="inline-flex items-center gap-2 px-4 py-2 bg-origin-slate rounded-full text-sm font-semibold text-gray-700 border border-gray-200">
                      <svg className="w-4 h-4 text-or-gold" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                      {loc.hours}
                    </div>
                  )}
                  ...
                  {loc.open && (
                    <div className="mt-4">
                      <span className="inline-block px-5 py-2 bg-or-gold text-white text-sm font-semibold rounded-lg">Shop Now</span>
                    </div>
                  )}
                </Link>
              ))}
          ...
          <div className="text-center mt-6">
            <Link to="/locations" className="inline-flex items-center gap-2 text-sm font-semibold text-or-gold hover:text-or-gold-dark transition-colors">View All Locations &rarr;</Link>
          </div>
        </div>
      </section>

      {/* ══════════ BEGIN YOUR JOURNEY CTA ══════════ */}
      <section className="py-16 sm:py-24 relative overflow-hidden" style={{ background: 'linear-gradient(160deg, #C9A84C 0%, #8B6914 40%, #1A1A1A 100%)' }}>
        <div className="absolute inset-0 pointer-events-none" style={{ backgroundImage: LEAF_PATTERN, backgroundSize: '100px 100px' }} />
        <div className="max-w-3xl mx-auto px-4 text-center relative z-10">
          ...
          <Link
            to="/register"
            className="inline-flex items-center gap-2 px-8 py-4 text-sm font-bold uppercase tracking-wider rounded-xl hover:-translate-y-1 transition-all"
            style={{ background: 'linear-gradient(135deg, #E8C45A, #F0A500, #B8922D)', color: '#1A1A1A', boxShadow: '0 15px 40px rgba(212,175,55,0.3)' }}
          >
