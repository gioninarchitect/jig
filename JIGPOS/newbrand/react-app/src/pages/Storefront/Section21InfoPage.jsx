...
export default function Section21InfoPage() {
  ...
  return (
    <div>
      {/* Hero */}
      <section style={{ background: 'linear-gradient(135deg, #7C3AED 0%, #6D28D9 100%)' }} className="py-16 sm:py-24 border-b-[3px] border-jig-amber">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h1 className="font-heading text-4xl sm:text-5xl text-white uppercase mb-2">Section 21 Medical Cannabis</h1>
          <p className="text-[1.1rem] text-jig-purple-dark font-accent italic" style={{ color: 'rgba(244,240,230,0.8)' }}>
      ...
      <div className="max-w-[900px] mx-auto px-4 sm:px-8 py-12">
        {/* Info box */}
        <div className="rounded-r-[8px] p-6 mb-8" style={{ background: 'rgba(58,95,72,0.1)', borderLeft: '4px solid #7C3AED' }}>
          <h2 className="font-heading text-xl text-jig-purple uppercase mb-2">What is Section 21?</h2>
        ...
        {/* Warning box */}
        <div className="rounded-r-[8px] p-6 mb-10" style={{ background: 'rgba(212,175,55,0.1)', borderLeft: '4px solid #D97706' }}>
        ...
        {/* 7-Step Process */}
        <h2 className="font-heading text-[2rem] text-jig-purple uppercase mt-8 mb-6">The Process</h2>
        <div className="space-y-4 mb-12">
          {PROCESS_STEPS.map((step) => (
            ...
            >
              <div className="w-10 h-10 bg-jig-purple rounded-full flex items-center justify-center shrink-0">
                <span className="text-gray-100 font-bold text-lg leading-[40px]">{step.num}</span>
              </div>
              <div>
                <h3 className="font-semibold text-jig-purple-dark text-lg">{step.title}</h3>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-12">
          <div>
            <h2 className="font-heading text-xl text-jig-purple uppercase mb-4">Requirements</h2>
            <ul className="space-y-3">
              {['Valid Section 21 prescription from a registered doctor', 'South African ID document', 'Proof of residential address', 'Completed registration on our platform', 'Upload of approved prescription document'].map((req) => (
                <li key={req} className="flex gap-3 text-sm text-gray-600">
                  <svg className="w-5 h-5 text-jig-purple shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 512 512"><path d="M256 512A256 256 0 1 0 256 0a256 256 0 1 0 0 512zM369 209L241 337c-9.4 9.4-24.6 9.4-33.9 0l-64-64c-9.4-9.4-9.4-24.6 0-33.9s24.6-9.4 33.9 0l47 47L335 175c9.4-9.4 24.6-9.4 33.9 0s9.4 24.6 0 33.9z"/></svg>
                  <span>{req}</span>
                </li>
              ))}
            </ul>
          ...
          <div>
            <h2 className="font-heading text-xl text-jig-purple uppercase mb-4">Qualifying Conditions</h2>
        ...
        <div
          ...
          style={{ background: 'linear-gradient(135deg, #7C3AED 0%, #6D28D9 100%)', border: '3px solid #D97706' }}
        >
        ...
        {/* Consultation Form */}
        <div className="bg-white rounded-[16px] p-8 mb-12" style={{ border: '2px solid #7C3AED' }}>
          <h2 className="font-heading text-2xl text-jig-purple uppercase mb-6 text-center">Request a Consultation</h2>
          {consultStatus === 'success' && (
            <div className="text-center p-6 mb-6">
              <div className="text-4xl text-jig-purple mb-3">&#10003;</div>
              <h3 className="font-heading text-xl text-jig-purple mb-2">Request Submitted</h3>
              <p className="text-jig-purple-dark">Our team will contact you to arrange your consultation.</p>
            </div>
          )}
          {consultStatus !== 'success' && (
            <form onSubmit={handleConsult} className="space-y-4">
              <div>
                <label className="block text-jig-purple-dark font-semibold mb-1.5 text-sm">Full Name *</label>
                <input type="text" value={consultForm.name} onChange={(e) => setConsultForm((f) => ({ ...f, name: e.target.value }))} required className="w-full px-3 py-3 bg-white border border-jig-purple rounded-[8px] text-white focus:outline-none focus:border-jig-amber" />
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-jig-purple-dark font-semibold mb-1.5 text-sm">Email *</label>
                  <input type="email" value={consultForm.email} onChange={(e) => setConsultForm((f) => ({ ...f, email: e.target.value }))} required className="w-full px-3 py-3 bg-white border border-jig-purple rounded-[8px] text-white focus:outline-none focus:border-jig-amber" />
                </div>
                <div>
                  <label className="block text-jig-purple-dark font-semibold mb-1.5 text-sm">Phone</label>
                  <input type="tel" value={consultForm.phone} onChange={(e) => setConsultForm((f) => ({ ...f, phone: e.target.value }))} className="w-full px-3 py-3 bg-white border border-jig-purple rounded-[8px] text-white focus:outline-none focus:border-jig-amber" />
                </div>
              </div>
              <div>
                <label className="block text-jig-purple-dark font-semibold mb-1.5 text-sm">Condition</label>
                <select value={consultForm.condition} onChange={(e) => setConsultForm((f) => ({ ...f, condition: e.target.value }))} className="w-full px-3 py-3 bg-white border border-jig-purple rounded-[8px] text-white focus:outline-none focus:border-jig-amber">
              ...
              <div>
                <label className="block text-jig-purple-dark font-semibold mb-1.5 text-sm">Additional Information</label>
                <textarea value={consultForm.message} onChange={(e) => setConsultForm((f) => ({ ...f, message: e.target.value }))} rows={3} className="w-full px-3 py-3 bg-white border border-jig-purple rounded-[8px] text-white focus:outline-none focus:border-jig-amber resize-vertical" />
              </div>
              <label className="flex items-start gap-2 cursor-pointer text-sm text-jig-purple-dark">
                <input type="checkbox" checked={consent} onChange={(e) => setConsent(e.target.checked)} className="accent-jig-purple mt-0.5" />
                <span>I consent to being contacted regarding my Section 21 consultation request.</span>
              </label>
              <button type="submit" disabled={submitting || !consent} className="w-full py-3 bg-jig-purple text-gray-100 font-semibold rounded-[8px] hover:bg-jig-purple-dark hover:-translate-y-px transition-all disabled:opacity-50" style={{ boxShadow: '0 4px 12px rgba(58,95,72,0.3)' }}>
        ...
        <div className="rounded-[16px] p-8 mb-12 bg-white" style={{ border: '2px solid rgba(58,95,72,0.2)' }}>
          <h2 className="font-heading text-2xl text-jig-purple uppercase mb-4">Prescription Renewal</h2>
            ...
            <ul className="space-y-2 ml-4">
              {[
                ...
                <li key={item} className="flex gap-3 text-sm">
                  <svg className="w-4 h-4 text-jig-amber shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 512 512"><path d="M256 512A256 256 0 1 0 256 0a256 256 0 1 0 0 512zM369 209L241 337c-9.4 9.4-24.6 9.4-33.9 0l-64-64c-9.4-9.4-9.4-24.6 0-33.9s24.6-9.4 33.9 0l47 47L335 175c9.4-9.4 24.6-9.4 33.9 0s9.4 24.6 0 33.9z"/></svg>
                  <span>{item}</span>
                </li>
              ))}
            </ul>
        ...
        <div className="rounded-[16px] p-8 mb-12" style={{ background: 'rgba(58,95,72,0.05)', border: '1px solid rgba(58,95,72,0.15)' }}>
          <h2 className="font-heading text-2xl text-jig-purple uppercase mb-4">Legal & Privacy</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <h3 className="font-semibold text-jig-purple-dark mb-2">Data Protection</h3>
            ...
            <div>
              <h3 className="font-semibold text-jig-purple-dark mb-2">Legal Framework</h3>
              <p className="text-sm text-gray-600 leading-relaxed">
                Medical cannabis access in South Africa is regulated under Section 21 of the Medicines and Related Substances Act (Act 101 of 1965), administered by SAHPRA. JIG Craft Cannabis operates in full compliance with all applicable regulations.
              </p>
            </div>
            <div>
              <h3 className="font-semibold text-jig-purple-dark mb-2">Your Rights</h3>
              <p className="text-sm text-gray-600 leading-relaxed">
                You have the right to access, correct, or delete your personal data at any time. Contact our privacy officer at <a href="mailto:privacy@jig.cleva-ai.co.za" className="text-jig-amber font-semibold">privacy@jig.cleva-ai.co.za</a> for any data-related requests.
              </p>
            </div>
            <div>
              <h3 className="font-semibold text-jig-purple-dark mb-2">Responsible Use</h3>
              <p className="text-sm text-gray-600 leading-relaxed">
        ...
        <div className="text-center mb-12">
          <Link
            to="/register"
            className="inline-block px-8 py-4 bg-jig-purple text-gray-100 font-semibold rounded-[8px] hover:bg-jig-purple-dark hover:-translate-y-0.5 transition-all"
        ...
        {/* FAQ */}
        <h2 className="font-heading text-2xl text-jig-purple uppercase mb-6 text-center">Frequently Asked Questions</h2>
