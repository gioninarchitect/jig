  ...
  return (
    <DashboardLayout title="My Account" menuItems={MENU_ITEMS} user={user} onNavigate={setTab} activeItem={tab}>
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500">Your Origin Account</p>
      </div>
...
function PurchaseLimitWidget({ userId }) {
  ...
  return (
    <div className="mb-4 p-3 rounded-lg border border-or-gold/20 bg-origin-slate">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
            <div className={`h-full rounded-full ${dailyPercent > 80 ? 'bg-origin-red' : dailyPercent > 50 ? 'bg-or-gold' : 'bg-or-gold'}`} style={{ width: `${Math.min(dailyPercent, 100)}%` }} />
          </div>
        </div>
        <div>
          <div className="flex justify-between text-xs mb-1">
          ...
          <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
            <div className={`h-full rounded-full ${monthlyPercent > 80 ? 'bg-origin-red' : monthlyPercent > 50 ? 'bg-or-gold' : 'bg-or-gold'}`} style={{ width: `${Math.min(monthlyPercent, 100)}%` }} />
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2">
        <button onClick={() => setTrack('lifestyle')} className={`px-4 py-2 rounded-lg text-xs font-bold ${track === 'lifestyle' ? 'bg-or-gold text-white' : 'bg-gray-100 text-gray-500'}`}>Lifestyle</button>
        <button onClick={() => setTrack('medical')} className={`px-4 py-2 rounded-lg text-xs font-bold ${track === 'medical' ? 'bg-or-gold text-white' : 'bg-gray-100 text-gray-500'}`}>Section 21</button>
      </div>

      {loading ? (
        <div className="flex justify-center py-8"><div className="w-6 h-6 border-2 border-or-gold border-t-transparent rounded-full animate-spin" /></div>
      ) : (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {products.map(p => (
            <div key={p._id} className="p-4 rounded-lg border border-gray-200 hover:border-or-gold/30 transition-colors">
              ...
              <div className="flex items-center justify-between mt-3">
                ...
                <button className="px-3 py-1.5 rounded-lg bg-or-gold text-white text-xs font-bold">Add to Cart</button>
              </div>
            </div>
          ))}
    ...
    <div className="space-y-4">
      ...
      {status ? (
        ...
        <div className="p-4 rounded-lg border border-gray-200 text-center">
          ...
          <button className="px-4 py-2 rounded-lg bg-or-gold text-white text-xs font-bold">Upload Prescription</button>
        </div>
      )}
    </div>
  );
