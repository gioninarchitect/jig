        ...
        <div className="flex h-full">
          ...
          <div className="flex-1 p-4 overflow-y-auto">
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
              {products.map(p => (
                <button key={p._id} onClick={() => addToCart(p)} className="p-3 rounded-lg border border-gray-700 bg-jig-slate/30 hover:bg-jig-slate/50 text-left transition-colors">
                  <div className="text-sm font-heading text-gray-100">{p.name}</div>
                  <div className="text-xs text-jig-amber mt-1">{formatCurrency(p.price)}</div>
                </button>
              ))}
            </div>
          </div>
          ...
          {/* Cart sidebar */}
          <div className="w-72 border-l border-gray-700 bg-jig-slate/20 flex flex-col">
              ...
              {cart.map(item => (
                <div key={item._id} className="flex items-center justify-between text-xs text-gray-100">
                  <div className="flex-1">
                    <div>{item.name}</div>
                    <div className="text-jig-amber">{item.quantity}x {formatCurrency(item.price)}</div>
                  </div>
                  <button onClick={() => removeFromCart(item._id)} className="text-jig-red text-[10px] hover:underline ml-2">Remove</button>
                </div>
              ))}
            ...
            <div className="p-3 border-t border-gray-700">
              ...
              <button onClick={submitOrder} disabled={cart.length === 0} className="w-full py-2.5 rounded-lg bg-jig-amber text-white text-sm font-bold disabled:opacity-30">
                Submit Order
              </button>
            </div>
          </div>
        ...
        <div className="p-6">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {queue.filter(o => o.status === 'ready').map(o => (
              <div key={o._id} className="p-6 rounded-lg border-2 border-jig-amber bg-jig-amber/10 text-center">
                <div className="font-heading text-3xl text-jig-amber">{o.orderNumber || `#${o._id?.slice(-6)}`}</div>
