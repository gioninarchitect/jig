  ...
  return (
    <div className="space-y-4">
      ...
      <div className="text-center">
        <div className={`inline-flex items-center justify-center w-12 h-12 rounded-full ${isIn ? 'bg-jig-purple/10' : 'bg-jig-red/10'}`}>
          <svg className={`w-6 h-6 ${isIn ? 'text-jig-purple' : 'text-jig-red'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
            {isIn
        ...
        <p className="text-xs text-jig-purple-light mt-1">
      ...
      <div>
        ...
        <div className="flex items-center gap-2">
          ...
          <input
            ...
            className="w-full px-3 py-2 rounded-lg border border-jig-purple/20 bg-white text-white text-lg font-heading outline-none focus:border-jig-amber"
            placeholder="0.00"
            step="0.01"
            min="0"
            autoFocus
        ...
        <div className="flex gap-2 mt-2">
          {QUICK_AMOUNTS.map(qa => (
            <button
              ...
              className="px-3 py-1 rounded-full bg-jig-slate border border-jig-purple/20 text-xs font-bold text-white hover:border-jig-amber transition-colors"
            >
              {formatCurrency(qa)}
            </button>
          ))}
        ...
        <div className="flex flex-wrap gap-1 mt-2">
          {REASONS.map(r => (
            <button
              key={r}
              onClick={() => setReason(r)}
              className={`px-2 py-1 rounded text-xs transition-colors ${reason === r ? 'bg-jig-purple text-white' : 'bg-jig-slate text-jig-purple-light hover:text-white'}`}
            >
              {r}
            </button>
          ))}
