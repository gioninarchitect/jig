  ...
  if (items.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16" style={{ color: '#64748b' }}>
        <svg className="w-12 h-12 mb-3 opacity-40" fill="none" stroke="#F0A500" viewBox="0 0 24 24">
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="px-4 py-3 flex items-center justify-between" style={{ borderBottom: '2px solid #334155' }}>
        ...
        <span className="text-xs font-semibold" style={{ color: '#F0A500' }}>{totals.itemCount} items</span>
      </div>
        ...
        {items.map((item, index) => (
          <div key={item._id + '-' + index} className="flex items-center justify-between p-3 rounded-lg" style={{ background: '#1e293b', border: '1px solid #334155' }}>
            <div className="flex-1 min-w-0 mr-3">
              ...
              <div className="text-xs" style={{ color: '#F0A500' }}>{formatCurrency(item.price)} each</div>
            </div>
            <div className="flex items-center gap-1">
              <button
                onClick={() => onDecrement(index)}
                className="flex items-center justify-center transition-all hover:opacity-80 active:scale-90"
                style={{ width: 28, height: 28, borderRadius: 6, background: '#F0A500', color: '#1a1a2e', fontWeight: 700, fontSize: '1rem', border: 'none' }}
              >-</button>
              <input
                type="number"
                value={item.quantity}
      ...
      <div className="px-4 py-3 space-y-1" style={{ background: '#1e293b', borderTop: '2px solid #334155' }}>
        ...
        <div className="flex justify-between font-heading uppercase pt-2" style={{ fontSize: '1.8rem', color: '#F0A500', borderTop: '2px solid #334155' }}>
          <span>Total</span>
      ...
      <div className="p-3 flex flex-col gap-2" style={{ background: '#0f172a', borderTop: '2px solid #F0A500' }}>
        <button
          onClick={onCheckout}
          ...
          style={{
            background: 'linear-gradient(135deg, #F0A500 0%, #B8922D 100%)',
            color: '#1a1a2e',
            fontWeight: 700,
            letterSpacing: '1px',
            border: 'none',
