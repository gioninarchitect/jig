...
function showToast(arg1, arg2, arg3) {
    ...
    document.querySelectorAll('.origin-shared-toast').forEach(t => t.remove());
    ...
    const colors = {
        success: { bg: Origin_CONFIG.BRAND.green, border: Origin_CONFIG.BRAND.gold },
        error: { bg: Origin_CONFIG.BRAND.darkRed, border: Origin_CONFIG.BRAND.red },
        warning: { bg: '#7A6520', border: Origin_CONFIG.BRAND.gold },
        info: { bg: Origin_CONFIG.BRAND.darkGreen, border: Origin_CONFIG.BRAND.green }
    };

    const c = colors[type] || colors.info;

    const toast = document.createElement('div');
    toast.className = 'origin-shared-toast';
    ...
    toast.innerHTML = `
        <div style="display:flex;align-items:center;gap:12px;">
            ...
            <button onclick="this.closest('.origin-shared-toast').remove()" style="background:none;border:none;color:inherit;font-size:1.25rem;cursor:pointer;padding:4px;line-height:1;flex-shrink:0;">&times;</button>
        </div>
    `;

    toast.style.cssText = `
        position:fixed;top:20px;right:20px;
        background:${c.bg};color:${Origin_CONFIG.BRAND.cream};
        border-left:4px solid ${c.border};
        padding:14px 18px;border-radius:10px;
        box-shadow:0 8px 32px rgba(0,0,0,0.3);
        z-index:200000;min-width:280px;max-width:420px;
        animation:jigToastIn 0.3s ease-out;
        font-family:'Barlow',sans-serif;
    `;

    // Inject keyframes if not already present
    if (!document.getElementById('origin-toast-keyframes')) {
        const style = document.createElement('style');
        style.id = 'origin-toast-keyframes';
        style.textContent = `
            @keyframes jigToastIn { from { transform: translateX(100%); opacity: 0; } to { transform: translateX(0); opacity: 1; } }
            @keyframes jigToastOut { from { transform: translateX(0); opacity: 1; } to { transform: translateX(100%); opacity: 0; } }
        `;
    ...
    setTimeout(() => {
        toast.style.animation = 'jigToastOut 0.3s ease-in forwards';
...
function formatCurrency(amount) {
    return `${Origin_CONFIG.CURRENCY.symbol}${(Number(amount) || 0).toFixed(2)}`;
}
...
async function apiCall(endpoint, method, data) {
    ...
    const url = endpoint.startsWith('http') ? endpoint : `${Origin_CONFIG.API_URL}${endpoint}`;
