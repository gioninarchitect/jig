/**
 * JIG Core Library — Unified namespace for all shared functionality
 * Requires (in order):
 *   1. frontend/config.js      (JIG_CONFIG, API_URL, DEV_MODE)
 *   2. frontend/jig-utils.js   (showToast, getToken, apiCall, logout, formatCurrency)
 *   3. frontend/jig-auth.js    (OTP helpers, token setters, isAuthenticated)
 *
 * Provides: window.JIG — single API surface for all pages and future React bridge.
 */
 ...
 * @returns {boolean}
 */
function _jigHasRole() {
    var user = _jigGetUser();
 ...
 */
async function _jigLogin(email, otpCode) {
    try {
 ...
 */
function _jigShowConfirm(message, onConfirm, options) {
    if (!options) options = {};
    ...
    overlay.style.cssText = 'position:fixed;top:0;left:0;width:100%;height:100%;' +
        ...
        'z-index:300000;font-family:"Inter",sans-serif;animation:jigConfirmIn 0.2s ease-out;';
 ...
 */
function _jigShowPrompt(message, onSubmit, options) {
    if (!options) options = {};
 ...
 */
function _jigShowLoading(message) {
...
function _jigFormatDate(d) {
    return new Date(d).toLocaleDateString('en-ZA');
}

function _jigFormatWeight(g) {
    return g >= 1000 ? (g / 1000).toFixed(1) + 'kg' : g + 'g';
}

function _jigFormatPercentage(n) {
    return n.toFixed(1) + '%';
}


// ============================================
// JIG Namespace — single API surface
// ============================================
...
var JIG = {
    ...
    ui: {
        showToast: showToast,
        showModal: _jigShowModal,
        hideModal: _jigHideModal,
        showConfirm: _jigShowConfirm,
        showPrompt: _jigShowPrompt,
        showLoading: _jigShowLoading,
        hideLoading: _jigHideLoading
    },

    format: {
        currency: formatCurrency,
        date: _jigFormatDate,
        weight: _jigFormatWeight,
        percentage: _jigFormatPercentage
    },
