...
function initWebSocket() {
    ...
    socket.on('payment:approved', (data) => {
        ...
        showNotification(
            ...
            () => {
                ...
                if (data.orderId) {
                    window.location.href = `/retail-store-staff.html?orderId=${data.orderId}`;
                }
            }
        );
        ...
        // If on retail-store staff page, refresh queue
        if (window.location.pathname.includes('retail-store-staff')) {
    ...
    socket.on('payment:pending', (data) => {
        ...
        showNotification(
            ...
            'info',
            () => {
                window.location.href = `/retail-store-staff.html?tab=approvals`;
            }
        );
        ...
        // If on retail-store staff page, refresh pending approvals
        if (window.location.pathname.includes('retail-store-staff')) {
            if (typeof loadPendingApprovals === 'function') {
                loadPendingApprovals();
            }
    ...
    socket.on('queue:updated', (data) => {
        ...
        // If on retail-store staff page, refresh queue
        if (window.location.pathname.includes('retail-store-staff') ||
            window.location.pathname.includes('retail-store')) {
            if (typeof loadQueue === 'function') {
                loadQueue();
            }
        }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const staffPages = ['admin.html', 'retail-store-staff.html', 'dashboard.html'];
