    ...
    if (wholesaleOrders.length === 0) {
        ...
        content.innerHTML = `
            <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 1rem; margin-bottom: 1.5rem;">
                <div style="background: rgba(212,175,55,0.1); padding: 1rem; border-radius: 8px; text-align: center;">
                ...
                <div style="background: rgba(166,52,41,0.1); padding: 1rem; border-radius: 8px; text-align: center;">
                ...
                <div style="background: rgba(58,95,72,0.1); padding: 1rem; border-radius: 8px; text-align: center;">
                ...
                ${wholesaleOrders.slice(0, 10).map(order => `
                    <div style="background: white; border: 1px solid var(--gray-200); border-radius: 8px; padding: 1rem; margin-bottom: 0.75rem;">
                        <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 0.5rem;">
                            ...
                            <span style="padding: 0.25rem 0.75rem; border-radius: 12px; font-size: 0.75rem; background: ${order.status === 'completed' ? 'rgba(34,197,94,0.1)' : order.status === 'pending' ? 'rgba(212,175,55,0.1)' : 'rgba(99,102,241,0.1)'}; color: ${order.status === 'completed' ? '#22C55E' : order.status === 'pending' ? 'var(--gold-dark)' : '#6366F1'};">
                ...
                ${wholesaleCustomers.map(customer => `
                    <div style="background: white; border: 1px solid var(--gray-200); border-radius: 8px; padding: 1rem; margin-bottom: 0.75rem;">
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            ...
                            <span style="padding: 0.25rem 0.75rem; border-radius: 12px; font-size: 0.75rem; background: ${customer.status === 'active' ? 'rgba(34,197,94,0.1)' : 'rgba(212,175,55,0.1)'}; color: ${customer.status === 'active' ? '#22C55E' : 'var(--gold-dark)'};">
