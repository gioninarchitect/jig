...
async function loadOrders() {
    ...
    try {
        ...
        orders.forEach((order, index) => {
            ...
            html += `
                <tr>
                    <td><strong>${order.orderNumber || 'DBC' + index}</strong></td>
        ...
        <div style="padding: 20px; background: var(--cream); border: 1px solid var(--green); border-radius: 10px; margin-bottom: 20px;">
            ...
            ${hasPOP ? `
                ${(() => {
                    ...
                    return `<div style="margin-top: 15px; padding: 15px; background: rgba(58, 95, 72, 0.1); border: 2px solid var(--green); border-radius: 8px; cursor: pointer;" onclick="viewProofModal('${fullUrl}')">
