...
# Check if bmh database exists
mongosh --eval "db.getMongo().getDBNames()" --quiet | grep bmh
```

**Purpose**: Check MongoDB setup and ensure bmh database doesn't already exist.
...
ls -la /var/www/bmh 2>/dev/null || echo "Directory does not exist"
...
ls -la /var/www/
ls -la /var/www/bmh 2>/dev/null || echo "CBD Wellness directory does not exist (GOOD)"
echo ""
...
- ✅ **ONLY** create new files in `/var/www/bmh`
- ✅ **ONLY** add new Nginx config (not modify existing)
- ✅ **ONLY** create new MongoDB database `bmh`
- ✅ **ONLY** add new PM2 app `bmh`
