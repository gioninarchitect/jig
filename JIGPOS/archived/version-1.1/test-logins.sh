...
ADMIN_RESPONSE=$(curl -s -X POST $API_URL/auth/login -H "Content-Type: application/json" -d '{"email":"admin@cbdwellness24.co.za","password":"Admin123!"}')
...
MANAGER_RESPONSE=$(curl -s -X POST $API_URL/auth/login -H "Content-Type: application/json" -d '{"email":"manager@cbdwellness24.co.za","password":"Manager123!"}')
...
ASSISTANT_RESPONSE=$(curl -s -X POST $API_URL/auth/login -H "Content-Type: application/json" -d '{"email":"assistant@cbdwellness24.co.za","password":"Assistant123!"}')
...
USER_RESPONSE=$(curl -s -X POST $API_URL/auth/login -H "Content-Type: application/json" -d '{"email":"user@cbdwellness24.co.za","password":"User123!"}')
...
PATIENT_RESPONSE=$(curl -s -X POST $API_URL/auth/login -H "Content-Type: application/json" -d '{"email":"patient@cbdwellness24.co.za","password":"Patient123!"}')
