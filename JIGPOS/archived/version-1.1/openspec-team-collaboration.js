...
/**
 * CBD Wellness 24 - Drive-Through Module
...
async function kickoffMeeting() {
    ...
    await say('pm', 'Good morning everyone! Welcome to the Drive-Through Module kickoff.', 500);
    await say('pm', 'Our goal: Build a 24/7 medical cannabis drive-through system.', 800);
    await sleep(600);
    ...
    await say('devops', 'I\'ll use our recent portal.cbdwellness24.co.za deployment as the benchmark.', 600);
...
async function businessAnalystPhase() {
    ...
    await action('ba', 'is connecting to MongoDB (cbdwellness24)...', 500);
    await sleep(800);
    ...
    await say('compliance', 'Good. The drive-through MUST integrate with that exact flow.', 700);
    await sleep(600);
...
`);
  ...
  ${colors.success}✓${colors.reset} ${colors.bright}Target Market:${colors.reset}
    ...
    - CBD wellness customers
    - Target: 50+ drive-through orders per day within 3 months
...
`);
    ...
    await outputFile('ba', 'openspec/drive-through-module/docs/business-requirements.md');
    await sleep(600);
...
async function productManagerPhase() {
    ...
    await say('pm', 'Vision: Industry-leading 24/7 medical cannabis drive-through experience.', 800);
    await say('pm', 'USP: Fastest, most compliant drive-through in South Africa.', 800);
    await sleep(600);
...
`);
    ...
    await outputFile('pm', 'openspec/drive-through-module/docs/product-requirements.md');
    await sleep(600);
...
`);
    ...
    console.log(`
  ${colors.success}Story DT-001:${colors.reset} ${colors.bright}Customer - Place Drive-Through Order${colors.reset}

  ${colors.dim}As a${colors.reset} medical cannabis patient,
  ${colors.dim}I want to${colors.reset} place a drive-through order via my mobile phone,
  ...
  ${colors.success}✓${colors.reset} And I select "Drive-Through Pickup"
  ${colors.success}✓${colors.reset} And I complete payment via InstaPay
  ${colors.success}✓${colors.reset} Then my order enters the drive-through queue
  ...
  ${colors.dim}As a${colors.reset} customer in the drive-through queue,
  ...
  ${colors.info}Acceptance Criteria:${colors.reset}
  ${colors.success}✓${colors.reset} Given I have an active drive-through order
...
`);
    ...
    await outputFile('sm', 'openspec/drive-through-module/docs/user-stories.md');
    await outputFile('sm', 'openspec/drive-through-module/docs/sprint-plan.md');
    await sleep(600);
...
async function designPhase() {
  ...
  1. ${colors.info}Order Placement${colors.reset}
     ...
     • "Drive-Through Pickup" prominent button
...
`);
  ...
  1. ${colors.info}Mobile Order Flow${colors.reset}
     ...
     • One-tap drive-through selection
...
`);
    ...
    await outputFile('uxWeb', 'openspec/drive-through-module/design/web-wireframes.md');
    await outputFile('uxMobile', 'openspec/drive-through-module/design/mobile-wireframes.md');
    await outputFile('uxWeb', 'openspec/drive-through-module/design/design-system.md');
    await sleep(600);
...
async function architectPhase() {
    ...
    await say('architect', 'Microservices approach. Drive-Through module as standalone service.', 800);
    await sleep(500);
...
`);
    ...
    await say('dev', 'What about the database schema for drive-through orders?', 700);
...
`);
    ...
    console.log(`
  ${colors.bright}Drive-Through API Endpoints:${colors.reset}

  ${colors.success}POST${colors.reset} ${colors.info}/api/v1/drive-through/order${colors.reset}
  → Create new drive-through order
  Body: { products[], paymentMethod, customerLocation }
  Response: { orderId, queuePosition, eta, paymentUrl }

  ${colors.success}GET${colors.reset} ${colors.info}/api/v1/drive-through/queue${colors.reset}
  → Get current queue status
  Response: { totalOrders, estimatedWait, activeOrders[] }

  ${colors.success}GET${colors.reset} ${colors.info}/api/v1/drive-through/orders/:orderId${colors.reset}
  → Track specific order
  Response: { order, queuePosition, eta, status }

  ${colors.success}PUT${colors.reset} ${colors.info}/api/v1/drive-through/location${colors.reset}
  → Update customer GPS location
  Body: { orderId, latitude, longitude, accuracy }
  Response: { updated, eta, distanceToShop }

  ${colors.success}POST${colors.reset} ${colors.info}/api/v1/drive-through/arrival${colors.reset}
  → Customer arrived at pickup window
  Body: { orderId, laneNumber }
  Response: { confirmed, laneAssignment, staffNotified }

  ${colors.success}POST${colors.reset} ${colors.info}/api/v1/drive-through/staff/verify${colors.reset}
  → Staff verify Section 21 compliance
  Body: { orderId, idDocument, prescriptionId }
  Response: { verified, auditTrailId, canHandover }

  ${colors.success}POST${colors.reset} ${colors.info}/api/v1/drive-through/complete${colors.reset}
...
`);
    ...
    await outputFile('architect', 'openspec/drive-through-module/specs/technical-design.md');
    await outputFile('architect', 'openspec/drive-through-module/specs/api-specification.md');
    await outputFile('architect', 'openspec/drive-through-module/specs/database-schema.md');
    await sleep(600);
...
`);
  ...
  ${colors.info}tests/drive-through/${colors.reset}
  ├── e2e/
...
`);
    ...
    await say('devops', 'Infrastructure ready. Using portal.cbdwellness24.co.za deployment as benchmark.', 900);
    await sleep(1000);
}
...
`);
    ...
    console.log(`\n${colors.info}All documentation saved to:${colors.reset} ${colors.bright}openspec/drive-through-module/${colors.reset}\n`);
}
