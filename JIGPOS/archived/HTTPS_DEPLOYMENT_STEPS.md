# Basotho Medical Herbs - HTTPS Deployment with Let's Encrypt
...
- **portal.basothomedicalherbs.ls** → 154.66.197.104 (A record)

You can verify with:
```bash
dig portal.basothomedicalherbs.ls +short
...
```bash
sudo cat /etc/nginx/sites-available/portal.basothomedicalherbs.ls
```
...
server {
    ...
    ssl_certificate /etc/letsencrypt/live/portal.basothomedicalherbs.ls/fullchain.pem; # managed by Certbot
    ssl_certificate_key /etc/letsencrypt/live/portal.basothomedicalherbs.ls/privkey.pem; # managed by Certbot
    include /etc/letsencrypt/options-ssl-nginx.conf; # managed by Certbot
    ssl_dhparam /etc/letsencrypt/ssl-dhparams.pem; # managed by Certbot
}

server {
    if ($host = portal.basothomedicalherbs.ls) {
...
# Location: https://portal.basothomedicalherbs.ls/
```
...
echo | openssl s_client -servername portal.basothomedicalherbs.ls -connect portal.basothomedicalherbs.ls:443 2>/dev/null | openssl x509 -noout -dates
```
...
1. Upload comingsoon.html to basothomedicalherbs.ls via FTP
2. Visit: https://basothomedicalherbs.ls/comingsoon.html
...
### Production URLs
- **Coming Soon**: https://basothomedicalherbs.ls/comingsoon.html
...
- **API**: https://portal.basothomedicalherbs.ls/api/v1/
...
```javascript
const PRODUCTION_API_URL = "https://portal.basothomedicalherbs.ls/api/v1";
```
...
# Test port 80
curl http://portal.basothomedicalherbs.ls
```

### Mixed Content Warnings

If you see mixed content warnings, ensure comingsoon.html uses HTTPS:
- API URL should be `https://portal.basothomedicalherbs.ls/api/v1`
...
```bash
# Export leads
mongoexport --db bmh --collection leads --out /tmp/leads-backup-$(date +%Y%m%d).json

# Full database backup
mongodump --db bmh --out /tmp/mongodb-backup-$(date +%Y%m%d)
