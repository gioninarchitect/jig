 ...
 * Tests the entire retail-store EFT payment approval workflow including:
...
test.describe('Retail Store Complete EFT Approval Flow', () => {
    ...
    test('End-to-end EFT payment approval workflow', async () => {
        ...
        // Fill login form on retail-store page
        await customerPage.fill('input[type="email"], input[name="email"]', 'user@basothomedicalherbs.ls');
        ...
        if (loginFormVisible) {
            await staffPage.fill('input[type="email"], input[name="email"]', 'manager@basothomedicalherbs.ls');
        ...
        if (paymentRowVisible) {
            ...
            try {
                const apiResponse = await staffPage.evaluate(async ({ orderId, paymentRef }) => {
                    ...
                    const response = await fetch(`http://localhost:3001/api/v1/retail-store/approvals/${orderId}/approve`, {
                        method: 'POST',
                        headers: {
