# Basotho Medical Herbs - Production Deployment Guide

## Server Information
- **IP Address**: 154.66.197.104
- **Domain**: portal.basothomedicalherbs.ls
- **Backend Port**: 3001
...
- CORS allows requests from basothomedicalherbs.ls domains
...
# 5. Check if bmh database exists
mongosh bmh --eval "db.stats()"
...
module.exports = {
  apps: [{
    ...
    error_file: '/var/www/bmh/logs/error.log',
    out_file: '/var/www/bmh/logs/out.log',
    log_file: '/var/www/bmh/logs/combined.log',
    time: true,
    watch: false,
...
mongosh bmh --eval "db.leads.find().sort({createdAt:-1}).limit(1).pretty()"
```
...
Upload `comingsoon.html` to wherever basothomedicalherbs.ls is hosted.
...
1. Open browser: `http://basothomedicalherbs.ls/comingsoon.html`
...
mongorestore --db bmh /tmp/mongodb-backup-20241104/bmh/

# Import specific collection
mongoimport --db bmh --collection leads --file /tmp/leads-backup-20241104.json
```
...
2. Restart PM2 app: `pm2 restart bmh`
...
mongosh bmh --eval "db.leads.find().count()"
```
...
pm2 show bmh | grep uptime

# Check error logs
pm2 logs bmh --err --lines 20
...
```bash
# Backup MongoDB
mongodump --db bmh --out /backups/mongodb-$(date +%Y%m%d)
...
### Key Files
- **App Directory**: `/var/www/bmh`
- **PM2 Config**: `/var/www/bmh/ecosystem.config.js`
- **Environment**: `/var/www/bmh/.env`
- **Logs**: `/var/www/bmh/logs/`
...
# View latest leads
mongosh bmh --eval "db.leads.find().sort({createdAt:-1}).limit(5).pretty()"
