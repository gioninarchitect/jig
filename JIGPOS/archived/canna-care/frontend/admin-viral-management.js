...
async function loadViralStats() {
  try {
    ...
    const affiliatesResponse = await fetch(`${API_URL}/admin/affiliates`, {
    ...
    // Load viral scores
    const scoresResponse = await fetch(`${API_URL}/viral/scores`, {
      headers: { 'Authorization': `Bearer ${token}` }
    });
    const scoresData = await scoresResponse.json();

    const totalScore = scoresData.scores?.reduce((sum, s) => sum + (s.viralScore || 0), 0) || 0;
...
function displayCampaigns(campaigns) {
  ...
  tbody.innerHTML = campaigns.map(campaign => `
    <tr>
      ...
      <td>
        <button class="action-btn-small" onclick="viewCampaign('${campaign._id}')" style="background: #2196F3;">View</button>
        <button class="action-btn-small" onclick="editCampaign('${campaign._id}')" style="background: #4CAF50;">Edit</button>
      </td>
    </tr>
  `).join('');
}
  ...
  tbody.innerHTML = influencers.map(inf => {
    ...
    return `
      <tr>
        ...
        <td>
          <button class="action-btn-small" onclick="viewInfluencer('${inf._id}')" style="background: #2196F3;">View</button>
        </td>
      </tr>
    `;
  }).join('');
...
async function loadProductScores() {
  try {
    const token = sessionStorage.getItem('adminToken');
    const response = await fetch(`${API_URL}/viral/scores`, {
      headers: { 'Authorization': `Bearer ${token}` }
    });

    const data = await response.json();
    allProductScores = data.scores || [];
  ...
  tbody.innerHTML = scores.slice(0, 50).map(score => {
    ...
    return `
      <tr>
        ...
        <td>
          <button class="action-btn-small" onclick="viewProductScore('${score._id}')" style="background: #2196F3;">View</button>
        </td>
      </tr>
    `;
  }).join('');
