...
const verifyAffiliateToken = (req, res, next) => {
  ...
  try {
    ...
    console.log('[verifyAffiliateToken] Decoded token:', decoded);
    req.affiliateId = decoded.affiliateId;
    req.user = decoded; // Also set req.user for admin routes
    console.log('[verifyAffiliateToken] req.user set to:', req.user);
    next();
  } catch (error) {
    console.error('[verifyAffiliateToken] JWT verification failed:', error.message);
...
router.get('/admin/all', verifyAffiliateToken, async (req, res) => {
  try {
    console.log('[GET /admin/all] req.user:', req.user);
    console.log('[GET /admin/all] req.user.id:', req.user?.id);
