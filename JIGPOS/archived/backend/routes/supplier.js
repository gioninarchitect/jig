// Supplier Routes - Supplier management and compliance
...
router.get('/expiring-licenses', authenticateToken, requireRole(['admin', 'staff_manager']), async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      message: 'Error fetching suppliers with expiring licenses'
    });
  }
});

// Get suppliers by product category
router.get('/by-category/:category', authenticateToken, requireRole(['admin', 'staff_manager']), async (req, res) => {
  try {
...
router.get('/:id/orders', authenticateToken, requireRole(['admin', 'staff_manager']), async (req, res) => {
  try {
...
router.get('/:id/batches', authenticateToken, requireRole(['admin', 'staff_manager']), async (req, res) => {
  try {
    const { page = 1, limit = 20, status } = req.query;
    const skip = (parseInt(page) - 1) * parseInt(limit);
    ...
    res.status(500).json({
      success: false,
      message: 'Error fetching supplier batches'
    });
  }
});
...
// Suspend supplier
router.post('/:id/suspend', authenticateToken, requireRole(['admin']), async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      message: 'Error suspending supplier'
    });
  }
});
...
router.post('/:id/reactivate', authenticateToken, requireRole(['admin']), async (req, res) => {
  try {
    ...
    if (supplier.complianceStatus === 'blacklisted') {
      return res.status(400).json({
        success: false,
        message: 'Cannot reactivate a blacklisted supplier'
      });
    }
...
router.get('/:id/performance', authenticateToken, requireRole(['admin', 'staff_manager']), async (req, res) => {
  try {
    const supplier = await Supplier.findById(req.params.id);
