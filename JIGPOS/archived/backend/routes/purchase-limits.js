// Purchase Limits Routes - Patient purchase limit tracking and enforcement
...
router.get('/patients/:id/limits', authenticateToken, requireRole(['admin', 'staff_manager', 'staff_assistant']), async (req, res) => {
  try {
    ...
    // Get limits from user or use defaults
    const limits = patient.purchaseLimits || {
      dailyLimit: 150,
      monthlyLimit: 600
    };
    ...
    res.status(500).json({
      success: false,
      message: 'Error fetching patient limits'
    });
  }
});

// Pre-check if a purchase is within limits
router.post('/patients/:id/check-purchase', authenticateToken, requireRole(['admin', 'staff_manager', 'staff_assistant']), async (req, res) => {
  try {
    ...
    if (!requestedGrams && !products) {
      return res.status(400).json({
        success: false,
        message: 'Either requestedGrams or products array is required'
      });
    }
    ...
    // Calculate total controlled grams from products if provided
    ...
    res.status(500).json({
      success: false,
      message: 'Error checking purchase limits'
    });
  }
});

// Get patient's purchase history
router.get('/patients/:id/purchase-history', authenticateToken, requireRole(['admin', 'staff_manager', 'staff_assistant']), async (req, res) => {
  try {
...
router.put('/patients/:id/limits', authenticateToken, requireRole(['admin']), async (req, res) => {
  try {
    ...
    if (!reason) {
      return res.status(400).json({
        success: false,
        message: 'Reason for limit change is required'
      });
    }
    ...
    if (dailyLimit !== undefined) {
      patient.purchaseLimits.dailyLimit = dailyLimit;
    }
    if (monthlyLimit !== undefined) {
      patient.purchaseLimits.monthlyLimit = monthlyLimit;
    }
    ...
    res.json({
      success: true,
      message: 'Patient limits updated successfully',
      limits: patient.purchaseLimits
    });
  } catch (error) {
    logger.error('Update patient limits error', { error: error.message, stack: error.stack });
    res.status(500).json({
      success: false,
      message: 'Error updating patient limits'
    });
  }
});

// Record a purchase (called after sale is completed)
router.post('/record', authenticateToken, requireRole(['admin', 'staff_manager', 'staff_assistant']), async (req, res) => {
  try {
    const {
      patient,
      order,
    ...
    if (!patient || !branch || !products) {
      return res.status(400).json({
        success: false,
        message: 'Patient, branch, and products are required'
      });
    }
    ...
    // Get limits
    const limits = patientDoc.purchaseLimits || {
      dailyLimit: 150,
      monthlyLimit: 600
    };

    // Calculate controlled grams
    ...
    // Get current usage for limit check record
...
router.post('/records/:id/void', authenticateToken, requireRole(['admin']), async (req, res) => {
  try {
    ...
    if (record.isVoided) {
      return res.status(400).json({
        success: false,
        message: 'Record is already voided'
      });
    }

    await record.void(req.user.id, reason);

    logger.info('Purchase record voided', {
      recordId: record._id,
      reason,
    ...
    res.json({
      success: true,
      message: 'Purchase record voided',
      record
    });
  } catch (error) {
    ...
    res.status(500).json({
      success: false,
      message: 'Error voiding purchase record'
    });
  }
});

// Get summary of all patients approaching limits
router.get('/approaching-limits', authenticateToken, requireRole(['admin', 'staff_manager']), async (req, res) => {
  try {
    const { threshold = 80 } = req.query;

    // Get all patients with Section 21 approval
    ...
    res.status(500).json({
      success: false,
      message: 'Error fetching patients approaching limits'
    });
  }
});
