...
- [x] **E0b — PHARMACY-SIDE dispense (DONE 28 Jul, verified).** Ran on origin-pos :3008 via `/orders/create` (Order collection) — **off-till: 0 Sales created, day-end untouched**. Received `VAL-SL260316` + its COA into Potchefstroom (Batch `labCertificateUrl` → farm COA, qa-approved, batchId=VAL-SL260316). Proof patient + Section-21 doc → **pharmacist vetting** (section21-review path) approved → `section21Status=approved`. **Gate proven:** dispense **403 SECTION21_REQUIRED** before vetting → **201** after (Order CBD-1785249718704, stock 100→99). Proof product left active, clearly named "E0 PROOF".
- [ ] **E0c — the remaining wire (batch/COA + S21 doc onto the dispense).** The dispensed Order does NOT stamp `item.batch` / `labCertificateUrl` / `section21DocumentId` (verified `track=? s21doc=none`) — so the sold line can't trace back to VAL-SL260316. Fix (origin-pos, careful — same process as the till): add `batch`+`labCertificateUrl` to `Order.js` items sub-schema; in `order.controller.createOrder` resolve the approved batch, stamp it, link the S21 doc, and call `Batch.recordSale()`. This CLOSES the farm→pharmacy digital trace. **Flo-gated** (POS code change).
