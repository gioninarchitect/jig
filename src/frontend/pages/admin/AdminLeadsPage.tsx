/**
 * JIG Craft Cannabis - Admin Leads Page
 *
...
const TIER_COLORS: Record<string, string> = {
  A: 'bg-jig-green/15 text-green-400 border border-jig-green/25',
  B: 'bg-jig-purple/15 text-jig-purple-light border border-jig-purple/25',
  C: 'bg-jig-amber/15 text-amber-400 border border-jig-amber/25',
  D: 'bg-white/[0.06] text-jig-gray-500 border border-white/[0.1]',
};

const STATUS_COLORS: Record<string, string> = {
  new: 'bg-blue-500/15 text-blue-400 border border-blue-500/25',
  contacted: 'bg-jig-amber/15 text-amber-400 border border-jig-amber/25',
  qualified: 'bg-jig-green/15 text-green-400 border border-jig-green/25',
  converted: 'bg-jig-gradient text-jig-white',
...
export default function AdminLeadsPage() {
  ...
  return (
    <div className="p-6">
      <h1 className="mb-6 font-heading text-2xl font-bold uppercase tracking-wide text-jig-white">Lead Pipeline</h1>

      <div className="mb-6 flex flex-wrap items-center gap-4">
        <div className="flex items-center gap-2">
          <span className="font-heading text-[10px] font-semibold uppercase tracking-[0.15em] text-jig-gray-500">Tier:</span>
          {['', 'A', 'B', 'C', 'D'].map((t) => (
            <button
              key={t}
        ...
        <div className="flex items-center gap-2">
          <span className="font-heading text-[10px] font-semibold uppercase tracking-[0.15em] text-jig-gray-500">Status:</span>
            ...
            <tbody className="divide-y divide-white/[0.04]">
              {leadList.map((lead) => (
                <tr key={lead.id} className="transition-colors hover:bg-white/[0.02]">
                  <td className="px-4 py-3 font-medium text-jig-white">
                    {lead.businessName}
                  </td>
                  <td className="px-4 py-3">
                    <div className="text-jig-gray-300">{lead.contactName}</div>
                    <div className="text-xs text-jig-gray-500">{lead.email}</div>
                    <div className="text-xs text-jig-gray-500">{lead.phone}</div>
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`rounded px-2 py-0.5 font-heading text-[10px] font-bold uppercase tracking-wider ${
                        TIER_COLORS[lead.estimatedTier] || 'bg-white/[0.06] text-jig-gray-500'
                      }`}
                    >
                      {lead.estimatedTier}
                    </span>
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`rounded px-2 py-0.5 font-heading text-[10px] font-medium uppercase tracking-wider ${
                        STATUS_COLORS[lead.status] || 'bg-white/[0.06] text-jig-gray-500'
                      }`}
                    >
                      {lead.status}
                    </span>
                  </td>
                  <td className="max-w-xs truncate px-4 py-3 text-xs text-jig-gray-500">
                    {lead.notes}
                  </td>
                  <td className="px-4 py-3 text-xs text-jig-gray-500">
