/**
 * JIG Craft Cannabis - Client Verification Page
 *
...
export default function VerificationPage() {
  ...
  return (
    <div className="p-6">
      <h1 className="mb-2 font-heading text-2xl font-bold uppercase tracking-wide text-jig-white">
        Verification
      </h1>
      ...
      {/* Status banner */}
      {isVerified ? (
        <div className="mb-6 rounded-lg border border-jig-green/30 bg-jig-green/10 px-5 py-4">
        ...
        <div className="mb-6 rounded-lg border border-jig-amber/30 bg-jig-amber/10 px-5 py-4">
          <div className="flex items-center gap-3">
      ...
      <div className="mb-6">
        <div className="mb-2 flex items-center justify-between">
          <span className="font-heading text-[11px] font-semibold uppercase tracking-[0.12em] text-jig-gray-400">
            Progress
          </span>
          <span className="text-sm font-medium text-jig-gray-300">
            {approvedCount} of 5 required documents verified
          </span>
        </div>
        <div className="h-2 overflow-hidden rounded-full bg-white/[0.06]">
          <div
            className="h-full rounded-full bg-jig-gradient transition-all duration-500"
        ...
        <div className="space-y-3">
          {DOC_TYPES.map((dt) => {
            ...
            return (
              <div
                key={dt.key}
                className="rounded-lg border border-white/[0.06] bg-jig-slate p-4 transition-all hover:border-white/[0.1]"
              >
                <div className="flex items-start gap-4">
                  {/* Icon */}
                  <div className={`mt-0.5 flex h-10 w-10 shrink-0 items-center justify-center rounded-lg ${
                    doc?.status === 'approved'
                      ? 'bg-jig-green/15 text-green-400'
                      : doc?.status === 'rejected'
                        ? 'bg-red-500/15 text-red-400'
                        : doc?.status === 'pending'
                          ? 'bg-jig-amber/15 text-amber-400'
                          : 'bg-white/[0.04] text-jig-gray-500'
                  }`}>
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <h3 className="text-sm font-semibold text-jig-white">{dt.label}</h3>
                      {!dt.required && (
                        <span className="rounded bg-white/[0.06] px-1.5 py-0.5 font-heading text-[9px] font-medium uppercase tracking-wider text-jig-gray-500">
                          Optional
                        </span>
                      )}
                    </div>
                    <p className="mt-0.5 text-xs text-jig-gray-500">{dt.description}</p>
                    ...
                    {doc && (
                      <div className="mt-2">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs text-jig-gray-600">
                  ...
                  <div className="shrink-0">
                    ...
                    />
                    {doc?.status === 'approved' ? (
                      <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-jig-green/15">
                      ...
                      <button
                        ...
                        className={`rounded px-3 py-1.5 font-heading text-[11px] font-semibold uppercase tracking-wider transition-all disabled:opacity-50 ${
                          doc?.status === 'rejected'
                            ...
                            : doc?.status === 'pending'
                              ? 'border border-white/[0.12] bg-white/[0.04] text-jig-gray-400 hover:bg-white/[0.08]'
      ...
      <div className="mt-6 rounded-lg border border-white/[0.06] bg-white/[0.02] px-4 py-3">
        <p className="font-heading text-[10px] font-semibold uppercase tracking-[0.12em] text-jig-gray-500">
          File Requirements
        </p>
