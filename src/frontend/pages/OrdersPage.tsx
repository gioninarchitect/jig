/**
 * PureGro Premium Cannabis Care - Orders Page
 *
...
export default function OrdersPage() {
  ...
  return (
    <div className="p-6">
      <h1 className="mb-6 font-heading text-2xl font-bold uppercase tracking-wide text-pg-white">Orders</h1>

      {orderList.length === 0 ? (
        <div className="rounded-lg border border-white/[0.06] bg-pg-dark p-8 text-center">
          <p className="text-sm text-pg-gray-500">No orders yet.</p>
        </div>
      ) : (
        <div className="space-y-3">
          {orderList.map((order) => (
            <div
              key={order.id}
              className="rounded-lg border border-white/[0.06] bg-pg-dark transition-all hover:border-white/[0.1]"
            >
              <button
                ...
                <div className="flex items-center gap-3">
                  <span
                    className={`rounded px-2 py-0.5 font-heading text-[10px] font-semibold uppercase tracking-wider ${
                      STATUS_COLORS[order.status] || 'bg-white/[0.06] text-pg-gray-500'
                    }`}
                  >
                    {order.status}
                  </span>
                  <span
                    className={`rounded px-2 py-0.5 font-heading text-[10px] font-semibold uppercase tracking-wider ${
                      PAYMENT_COLORS[order.paymentStatus] || 'bg-white/[0.06] text-pg-gray-500'
                    }`}
                  >
                    {order.paymentStatus}
                  </span>
                  <span className="text-sm font-semibold text-pg-gold">
                    {formatRand(order.total)}
                  </span>
                  <Link
                    to={`/orders/${order.id}`}
                    onClick={(e) => e.stopPropagation()}
                    className="rounded border border-pg-green/25 bg-pg-green/10 px-2 py-0.5 font-heading text-[9px] font-semibold uppercase tracking-wider text-pg-green-light hover:bg-pg-green/20"
                  >
                    View
                  </Link>
                  <svg
                    className={`h-4 w-4 text-pg-gray-500 transition-transform ${
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody className="divide-y divide-white/[0.04]">
                      {order.items.map((item, i) => (
                        <tr key={i}>
                          <td className="py-1.5 text-pg-gray-300">{item.name || item.productName}</td>
                          <td className="py-1.5 text-right text-pg-gray-300">{item.quantity}</td>
                          <td className="py-1.5 text-right text-pg-gray-300">
                            R{safeNum(item.unitPrice).toFixed(2)}
                          </td>
                          <td className="py-1.5 text-right font-medium text-pg-white">
                    ...
                    <tfoot className="border-t border-white/[0.06]">
                      <tr>
                        <td colSpan={3} className="py-1.5 text-right text-xs text-pg-gray-500">
                          Subtotal
                        </td>
                        <td className="py-1.5 text-right text-sm text-pg-gray-300">
                      ...
                      <tr>
                        <td colSpan={3} className="py-1.5 text-right text-sm font-bold text-pg-white">
                          Total
                        </td>
                        <td className="py-1.5 text-right text-sm font-bold text-pg-gold">
