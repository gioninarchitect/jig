...
export default function DailyCheckPage() {
  ...
  const submitMut = useMutation({
    mutationFn: () => api.post('/daily-checks', { zone, items: itemStatus, notes }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['daily-checks'] });
      addToast('success', `${config.label} daily check submitted`);
      setShowSubmit(false); setItemStatus({}); setNotes('');
    },
      ...
      <Modal open={showSubmit} onClose={() => setShowSubmit(false)} title={`${config.label} · Daily Check`}>
        <div className="space-y-2.5">
          ...
          {config.items.map(item => (
            <div key={item} className="flex items-center justify-between gap-3 rounded-lg border border-white/10 bg-white/5 px-3.5 py-2.5">
              <span className="text-sm text-white">{item}</span>
              <div className="flex gap-1.5">
                <button
                  onClick={() => setItemStatus(s => ({ ...s, [item]: 'PASS' }))}
                  className={`w-10 h-9 rounded-lg text-xs font-bold transition ${
                    itemStatus[item] === 'PASS'
                      ? 'bg-green-500/80 text-white'
                      : 'bg-white/5 text-white/40 hover:bg-white/10'
                  }`}>✓</button>
                <button
                  onClick={() => setItemStatus(s => ({ ...s, [item]: 'FAIL' }))}
                  className={`w-10 h-9 rounded-lg text-xs font-bold transition ${
                    itemStatus[item] === 'FAIL'
          ...
          <ModalButton
            ...
            disabled={submitMut.isPending || !allItemsMarked}
          >
            {submitMut.isPending ? 'Submitting…' : allItemsMarked ? 'Submit check' : `Mark all ${config.items.length} items to submit`}
          </ModalButton>
